...
import { sendMessage, type ClaudeConfig } from "./claude"
...
describe("sendMessage", () => {
  ...
  it("returns error when API key is missing", async () => {
    const result = await sendMessage({ apiKey: "" }, [], "Hello")
  ...
  it("sends messages to Claude and returns response", async () => {
    ...
    const result = await sendMessage(mockConfig, mockMessages, "How are you?")
  ...
  it("uses custom model and max tokens when provided", async () => {
    ...
    await sendMessage(customConfig, [], "Hello")
  ...
  it("handles API errors gracefully", async () => {
    ...
    const result = await sendMessage(mockConfig, [], "Hello")
